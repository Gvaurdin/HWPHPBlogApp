...
class BlogDB
{
    public static function addPostDB() : string
    {
        $db =getDB();
        ...
        $nameAuthor = Utils::getInput("Введите имя автора поста: ", "Имя не может быть пустым");
        $surnameAuthor = Utils::getInput("Введите фамилию автора поста: ", "Фамилия не может быть пустой");
        ...
        if(!$user) {
            ...
            if (!$userType) {
                Helpers::errorHandle("Нет категории пользователя : {$userType}");
            }
        ...
        if ($existingPost) {
            Helpers::errorHandle("Пост с заголовком {$titleTrim} уже существует в БД");
        }
    ...
    public static function readAllPostsDB(): string
    {
        $db =getDB();
        ...
        if(empty($result)) {
            Helpers::errorHandle("Походу постов в БД нет!");
        }

        //формируем вывод
        $output = "Все посты из БД:" .PHP_EOL;

        return Utils::printPost($result, $output);
    }

    public static function readPostDB($id) : string
    {
        $db =getDB();
        ...
        if(!$result){
            Helpers::errorHandle("Пост с ID:{$id} не найден в БД");
        }
        return "Найден следующий пост(БД):". PHP_EOL .print_r($result,true) . PHP_EOL;
    }

    public static function searchPostDB(string $search) : string
    {
        $db =getDB();
        ...
        if(empty($result)) {
            Helpers::errorHandle("Посты по указанному поисковому слову : {$search} не найдены");
        }
   ...
   public static function deletePostDB($id) : string
    {
        $db =getDB();
        ...
        $result = $stmt->fetch();
        if(!$result){
            Helpers::errorHandle("Пост с ID: {$id} не найден в базе данных");
        }
        //удаляем пост
    ...
    public static function clearPostsDB() : string
    {
        $db =getDB();
