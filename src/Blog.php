...
use App\Blog\utils\Utils as Utils;
use App\Blog\config\Config as Config;
use App\Blog\Helpers as Helpers;
use Exception;

class Blog {
    public static function addPost() : string
    {
        try {
            //считываем заголовок
            $title = Utils::getInput("Введите заголовок поста:", "Заголовок не может быть пуст");
            $body = Utils::getInput("Введите текст поста:", "Пост не может быть пустым");
            ...
            $filePath = Config::getPostFilePath($post['id']);
            ...
            if (file_put_contents($filePath, json_encode($post, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)) === false) {
                Helpers::errorHandle("Сбой при записи поста в файл. Не удалось сохранить пост в файл");
            }
            ...
            $fileIndexPath = Config::getIndexFilePath();
            ...
            if (!file_exists($fileIndexPath) || filesize($fileIndexPath) == 0) {
                ...
                if ($fileIndex === false) {
                    Helpers::errorHandle("Не удалось открыть индексный файл для записи.");
                }
            ...
            if(!$fileIndex) {
                Helpers::errorHandle("Не удалось открыть индексный файл записи");
            }
    ...
    public static function readAllPosts(): string
    {
        try {
            ...
            if(!file_exists($fileIndexPath) || filesize($fileIndexPath) == 0) {
                Helpers::errorHandle("Индексный файл отсутствует или пустой: {$fileIndexPath}");
            }

            // открываем чтение файла индекса
            $fileIndex = fopen($fileIndexPath, 'r');
            if(!$fileIndex) {
                Helpers::errorHandle("Не удалось открыть индексный файл: {$fileIndexPath}");
            }
    ...
    public static function readPost($id) : string
    {
        try {
            ...
            while (($row = fgetcsv($fileIndex)) !== false) {
                ...
                if($id == $row[0]) {
                    ...
                    if(!file_exists($pathFile)) {
                        Helpers::errorHandle("Пост с ID {$id} не найден");
                    }
    ...
    public static function searchPost(string $search) : string
    {
        try {
            //получаем путь к директории с постами
            $dirPosts = Config::getDirectoryPosts();

            if(!is_dir($dirPosts)) {
                Helpers::errorHandle("Директория с постами не найдена: {$dirPosts}");
            }
            ...
            if(empty($results)) {
                return Helpers::errorHandle("Ничего не найдено по запросу: {$search}");
            }
    ...
    public static function deletePost($id) : string
    {
        try {
            //получаем путь к индексному файлу
            $fileIndexPath = Config::getIndexFilePath();
            if(!file_exists($fileIndexPath) || filesize($fileIndexPath) == 0) {
                return Helpers::errorHandle("Индексный файл отсутствует или пустой: {$fileIndexPath}");
            }

            //открываем индексный файл для чтения
            $fileIndex = fopen($fileIndexPath, 'r');
            if(!$fileIndex) {
                return Helpers::errorHandle("Не удалось открыть индексный файл: {$fileIndexPath}");
            }
            ...
            while (($row = fgetcsv($fileIndex)) !== false) {
                ...
                if($id == $row[0]) {
                    ...
                    if(file_exists($postFilePath)) {
                        ...
                        return Helpers::errorHandle("Пост с ID {$id} не найден в файле: {$postFilePath}");
                    }
            ...
            if (!$fileIndex) {
                return Helpers::errorHandle("Не удалось открыть индексный файл для записи: {$fileIndexPath}");
            }
    ...
    public static function clearPosts() : string
    {
        try {
            $dirPosts = Config::getDirectoryPosts();
            if(!is_dir($dirPosts)) {
                return Helpers::errorHandle("Директория с постами не найдена: {$dirPosts}");
            }
            $files = glob($dirPosts . '/*.json');
            ...
            if (file_put_contents($fileIndexPath, '') === false) {
                return Helpers::errorHandle("Не удалось очистить индексный файл.");
            }
